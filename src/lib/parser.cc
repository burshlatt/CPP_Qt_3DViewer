...
void Parser::Parse(std::string_view path) {
    ...
    if (file.is_open()) {
        ...
        if (data_.mtl.empty())
            data_.mtl.emplace_back(Mtl{});
        if (data_.usemtl.empty())
            data_.usemtl.emplace_back(UseMtl{});
...
void Parser::NormalizeVertexes() {
    ...
    auto future = std::async(std::launch::async, std::bind(&s21::Parser::SetUniqueEdges, this));
...
void Parser::ParseVertex(std::vector<float> &vert, int size) {
    ...
    for (int i = 1; i < size; i++) {
        ...
        vert.push_back(num);
        if (tokens_[0] == "v") {
            if (num < data_.min_coord[i - 1])
                data_.min_coord[i - 1] = num;
            if (num > data_.max_coord[i - 1])
                data_.max_coord[i - 1] = num;
        }
    }
}

void Parser::NormalizeWithEarcut(std::vector<Index> &ind_buff) {
    float off_01 = data_.vertexes_v[ind_buff[2].fv * 3 + 0] - data_.vertexes_v[ind_buff[0].fv * 3 + 0];
    float off_02 = data_.vertexes_v[ind_buff[2].fv * 3 + 1] - data_.vertexes_v[ind_buff[0].fv * 3 + 1];
    float off_03 = data_.vertexes_v[ind_buff[2].fv * 3 + 2] - data_.vertexes_v[ind_buff[0].fv * 3 + 2];
    float off_04 = data_.vertexes_v[ind_buff[3].fv * 3 + 0] - data_.vertexes_v[ind_buff[1].fv * 3 + 0];
    float off_05 = data_.vertexes_v[ind_buff[3].fv * 3 + 1] - data_.vertexes_v[ind_buff[1].fv * 3 + 1];
    float off_06 = data_.vertexes_v[ind_buff[3].fv * 3 + 2] - data_.vertexes_v[ind_buff[1].fv * 3 + 2];
...
Point3D Parser::Normalize(const Point3D& e) noexcept {
...
Point3D Parser::WorldToLocal(const Point3D& a, const Point3D& u, const Point3D& v, const Point3D& w) noexcept {
    return {Dot(a, u), Dot(a, v), Dot(a, w)};
}

void Parser::NormalizeWithNewell(std::vector<Index> &ind_buff, unsigned int p_count) {
    Index i0{}, i0_2{};
    Point3D n1{};
...
void Parser::NormalizeIndices(std::vector<Index> &ind_buff, unsigned int p_count) {
    if (p_count == 4)
        NormalizeWithEarcut(ind_buff);
    else if (p_count > 4)
        NormalizeWithNewell(ind_buff, p_count);
    else
        std::move(ind_buff.begin(), ind_buff.end(), std::back_inserter(data_.indices));
}
        ...
        if (!it.empty() && it != "f") {
            ...
            if (it[0] == '/') {
                ...
                if (it[0] != '/') {
                    num = std::strtol(it.c_str(), &end, 10);
                    if (num < 0) {
                        ind.ft = data_.vertexes_t.size() / 2 - static_cast<unsigned int>(-num);
                    } else if (num > 0) {
            ...
            if (it[0] == '/') {
                ...
                num = std::strtol(it.c_str(), &end, 10);
                if (num < 0) {
                    ind.fn = data_.vertexes_n.size() / 3 - static_cast<unsigned int>(-num);
                } else if (num > 0) {
